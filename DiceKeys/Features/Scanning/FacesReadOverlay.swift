...
/// preview, each rotated to match its die. Before anything is read, shows the
...
struct FacesReadOverlay: View {
    ...
    var body: some View {
        if facesRead.isEmpty || imageFrameSize.width == 0 || imageFrameSize.height == 0 {
            ...
            // (clockwise angles in a y-down coordinate space); check on a device.
            Canvas { context, size in
                for faceRead in facesRead {
                    guard let faceSizeInFramePixels = faceRead.length,
                          let angleLandscape = faceRead.angle else {
                        continue
                    }
                    let faceSizeInPixels = faceSizeInFramePixels * size.width / imageFrameSize.width
                    let fontSize = faceSizeInPixels * FaceDimensionsFractional.fontSize
                    let font = Font.custom("Inconsolata-Bold", size: fontSize)
                    let centerLandscape = faceRead.center
                    // Frames arrive rotated by AVFoundation, so face coordinates are
                    // already in the preview's orientation; this used to rotate them 90°
                    // to undo the raw landscape-native buffer.
                    let angle = angleLandscape
                    let center = centerLandscape
                    let coordinateSystemFromCenterOfDie = AngularCoordinateSystem(
                        zeroPoint: CGPoint(
                            x: center.x * size.width / imageFrameSize.width,
                            y: center.y * size.height / imageFrameSize.height
                        ),
                        angle: angle,
                    ...
                    if let letter = faceRead.letter {
                        draw(letter.rawValue, in: context, at: coordinateSystemFromCenterOfDie.pointAt(offset: letterOffset), angle: angle, font: font)
                    }
                    if let digit = faceRead.digit {
                        draw(digit.rawValue, in: context, at: coordinateSystemFromCenterOfDie.pointAt(offset: digitOffset), angle: angle, font: font)
                    }
                }
            }
            .frame(width: renderedSize.width, height: renderedSize.height)
...
let sampleFacesReadJson: String = """
