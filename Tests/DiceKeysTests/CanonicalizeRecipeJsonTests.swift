//
//  CanonicalizeRecipeJsonTests.swift
//  DiceKeysTests
//
...
struct CanonicalizeRecipeJsonTests {
    ...
    @Test("Recipe JSON is canonicalized like the reference implementation",
          arguments: canonicalizationTestVector)
    func canonicalizeRecipeJson(vector: (input: String, expected: String)) {
        #expect(vector.input.canonicalizeRecipeJson() == vector.expected)
    }

    // Escaped strings come out exactly as they went in, as the reference keeps each
    // string's original quoted form.
    static let escapingTestVector: [String] = [
        #"{"purpose":"say \"hi\""}"#,
        #"{"purpose":"back\\slash"}"#,
        #"{"purpose":"line\nbreak\ttab\u0001"}"#,
    ...
    @Test("Quotes, backslashes and control characters stay escaped",
          arguments: escapingTestVector)
    func canonicalizeEscapedStrings(json: String) {
        #expect(json.canonicalizeRecipeJson() == json)
    }
}
