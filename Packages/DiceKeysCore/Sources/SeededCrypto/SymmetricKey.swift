...
/// A symmetric (XSalsa20-Poly1305) key derived from a seed and a recipe.
public struct SymmetricKey: Sendable, Hashable {
    ...
    init(nativeJson json: String) throws {
        ...
        self.keyBytes = try Data.fromNativeHex(fields.keyBytes, field: "keyBytes")
    }

    public static func deriveFromSeed(withSeedString seedString: String, recipe: String) throws -> SymmetricKey {
        try SymmetricKey(nativeJson: Native.string { out, err in dkc_symmetric_key_derive(seedString, recipe, out, err) })
    }

    public static func from(json: String) throws -> SymmetricKey {
        try SymmetricKey(nativeJson: Native.string { out, err in dkc_symmetric_key_from_json(json, out, err) })
    }
    ...
    public func seal(with data: Data, unsealingInstructions: String = "") throws -> PackagedSealedMessage {
        let packaged = try Native.withBytes(data) { bytes, count in
            try Native.string { out, err in
                dkc_symmetric_key_seal(json, bytes, count, unsealingInstructions, out, err)
            }
        }
    ...
    public func unseal(withJsonPackagedSealedMessage packagedSealedMessageJson: String) throws -> Data {
        try Native.bytes { out, err in dkc_symmetric_key_unseal(json, packagedSealedMessageJson, out, err) }
    }
    ...
    public func unseal(ciphertext: Data, unsealingInstructions: String = "") throws -> Data {
        try Native.withBytes(ciphertext) { bytes, count in
            try Native.bytes { out, err in
                dkc_symmetric_key_unseal_ciphertext(json, bytes, count, unsealingInstructions, out, err)
            }
        }
    }
    ...
    public static func unseal(withJsonPackagedSealedMessage packagedSealedMessageJson: String, seedString: String) throws -> Data {
        try Native.bytes { out, err in dkc_seed_unseal_with_symmetric_key(seedString, packagedSealedMessageJson, out, err) }
    }
}
